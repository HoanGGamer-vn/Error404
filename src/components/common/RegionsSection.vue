...
const currentRegion = computed(() => props.regions[currentIndex.value] || props.regions[0])

function nextSlide() {
  if (isTransitioning.value) return
  isTransitioning.value = true
  setTimeout(() => {
    currentIndex.value = (currentIndex.value + 1) % props.regions.length
  }, 100)
  setTimeout(() => {
...
function previousSlide() {
  ...
  isTransitioning.value = true
  setTimeout(() => {
    currentIndex.value = (currentIndex.value - 1 + props.regions.length) % props.regions.length
  }, 100)
  setTimeout(() => {
    isTransitioning.value = false
  }, 800)
...
function getPrevIndex() {
  return (currentIndex.value - 1 + props.regions.length) % props.regions.length
}

function getNextIndex() {
  return (currentIndex.value + 1) % props.regions.length
}
